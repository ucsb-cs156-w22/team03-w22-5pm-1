...
describe("EarthquakesForm tests", () => {
  ...
  test("renders correctly when passing in a Earthquake ", async () => {
    const { getByText, getByTestId } = render(
      <Router>
        <EarthquakesForm
          initialEarthquakes={earthquakeFixtures.oneEarthquake}
        />
      </Router>
    );
    await waitFor(() =>
    ...
    await waitFor(() =>
      expect(getByTestId(/EarthquakesForm-dist/)).toBeInTheDocument()
    );
  ...
  test("Correct Error messages on bad input", async () => {
    ...
    expect(
      getByText(/Distance in km must be a number, e.g. 1.6km/)
    ).toBeInTheDocument();
  });
  ...
  test("Correct Error messages on missing input", async () => {
    ...
    await waitFor(() =>
      expect(
        getByText(/Minimum Magnitude of an earthquake is required./)
      ).toBeInTheDocument()
    );
    expect(
      getByText(/Distance in km from Storke Tower is required./)
    ).toBeInTheDocument();
  });
  ...
  test("No Error messages on good input", async () => {
    ...
    expect(
      queryByText(/Distance in km must be a number, e.g. 1.6km/)
    ).not.toBeInTheDocument();
  });
